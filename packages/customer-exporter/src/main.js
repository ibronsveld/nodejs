...
export default class CustomerExporter {
  ...
  static fetchObjects(
    ...
    const request = CustomerExporter.buildRequest(projectKey, predicate, store)
    ...
    return client.process(
      ...
      ({ statusCode, body }: Object): Promise<any> => {
        ...
        logger.debug(
          ...
          body.count > 1 ? 'customers' : 'customer'
        )
  ...
  static buildRequest(projectKey: string, predicate: ?string, store: ?string): ClientRequest {
    const uri = CustomerExporter.buildUri(projectKey, predicate, store)        
    return {
      uri,
      method: 'GET',
    }
  ...
  static buildUri(projectKey: string, predicate: ?string,  store: ?string): string {
    const requestBuilder = createRequestBuilder({
      ...
      customServices: {
        storeCustomers: {
          type: 'customers',
          endpoint: '/in-store/key=' + store + "/customers",
          features: [features.query, features.queryOne],
        }
      }
    })

    const service = (store ? requestBuilder.storeCustomers : requestBuilder.customers)
