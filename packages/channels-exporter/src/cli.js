...
import ChannelsExporter from './main'
import { description } from '../package.json'

process.title = 'channels-exporter'
...
${description}`
  ...
  .option('expand', {
    type: 'array',    
    describe: 'Specify reference expansion.',
  })
  .option('logLevel', {
    default: 'info',
...
resolveCredentials(args)
  .then((credentials) => {
    ...
    return new ChannelsExporter(exporterOptions)
  })
  .then((channelsExporter) => channelsExporter.run(args.output))
  .catch(errorHandler)
